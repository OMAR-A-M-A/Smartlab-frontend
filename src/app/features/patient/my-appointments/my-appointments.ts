...
export class MyAppointments implements OnInit {
  ...
  loadAppointments(): void {
    ...
    this.bookingService.getPatientAppointments().subscribe({
      next: (res) => {
      ...
      error: (err) => {
        this.isLoading = false;
        this.cdr.detectChanges();
        const errorMsg = err.error?.message || 'Failed to load appointments.';
  ...
  cancelAppointment(appt: MyAppointment): void {
    ...
    const normalizedDate = this.toISODate(appt.appointmentDate);
